...
- [ ] Implement version matching checks that identify when a file matches another feature version
...
- [ ] Implement conflict detection across features and surface actionable errors (Spec: Conflict
      Handling).
- [ ] Implement `explain` command to show feature ownership and drift reasons (Spec: CLI Commands).
- [ ] Add `--json` output mode for `projects` and `status` (Spec: Output & Logging).
