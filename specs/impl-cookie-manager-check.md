...
- [x] Remove feature versioning from config loaders and schemas; update config parsing to use
      ...
      Schema).
- [x] Simplify feature metadata handling to a single `files` list and delete support for
...
- [x] Add LLM prompt generation and embed it in the report output exactly as specified
      (Spec: LLM Prompt).
...
- [x] Update README CLI examples and exit codes to reflect the new `check`-only workflow
